...
public function cropImage($file, $size)
{
    ...
    public function onContentPrepare($context, &$article, &$params, $limitstart)
    {
        if (version_compare(JVERSION, '4.0', '>=')) {
            ...
            if (!$wa->assetExists('script', 'jquery')) {
                $wa->registerAndUseScript('jquery', Uri::root(true) . 'media/vendor/jquery/js/jquery.min.js', ['version' => 'auto', 'relative' => true]);
            }
            if (!$wa->assetExists('script', 'jquery-migrate')) {
                $wa->registerAndUseScript('jquery-migrate', Uri::root(true) . 'media/vendor/jquery-migrate/js/jquery-migrate.min.js', ['version' => 'auto', 'relative' => true]);
            }
        ...
        foreach ($allMatches as $m=>$match) {
            ...
            $html[$m] .= '<div class="' . ($this->params->get("gallery_type", "owl_carousel") == "owl_carousel" ? 'owl-carousel carrossel-' . $article->id . $m . ' owl-theme' : 'row') . '">';
            foreach ($files as $k => $file) {
                ...
                $thumbnail = $directory . '/thumbnail/' . $file;
                ...
                $html[$m] .= '<div class="item '.($this->params->get("gallery_type", "owl_carousel") == "owl_carousel" ? 'item-'.$article->id.$m.'-'.$k : 'col-md-'.round(12/$this->params->get("images_per_row")).'').' mb-3">';
                ...
                if ($this->params->get("gallery_type", "owl_carousel") == "grid") {
                    $html[$m] .= HTMLHelper::_('image', $thumbnail, $alt, ['class' => 'embed-responsive-item img-fluid lazyload', 'loading' => 'lazy']);
                } else {
                    $html[$m] .= HTMLHelper::_('image', $imagem, $alt, ['class' => 'embed-responsive-item img-fluid lazyload', 'loading' => 'lazy']);
                }
                  if ($this->params->get("modal", "1") == "1") {
                    $html[$m] .= '</div>';
                }
        ...
        foreach ($files as $k => $file) {
            ...
            $modalsHtml .= HTMLHelper::_('image', $imagem, $alt, ['class' => 'img-fluid lazyload', 'loading' => 'lazy']);
            // Botão Previous
